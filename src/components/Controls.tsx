  ...
  const handleGeolocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setGeolocationMessage(`Location found: ${latitude.toFixed(4)}, ${longitude.toFixed(4)}`);
  ...
  return (
    <div className="absolute right-4 top-1/2 transform -translate-y-1/2 z-5">
      <div className="flex flex-col space-y-2 bg-white/90 backdrop-blur-sm p-2 rounded-lg shadow-lg">
        <ControlButton onClick={handleZoomIn} tooltip="Zoom In">
          <ZoomIn size={20} />
        </ControlButton>
        
        <ControlButton onClick={handleZoomOut} tooltip="Zoom Out">
          <ZoomOut size={20} />
        </ControlButton>
